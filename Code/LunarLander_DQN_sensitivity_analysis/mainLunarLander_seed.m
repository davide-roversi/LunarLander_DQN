...
agent = NaN;
seed = [10, 71];
episodeRewardCell = cell(1,length(seed));
averageRewardCell = cell(1,length(seed));

for iterator = 1:length(seed)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "seed.jpg";
    settings.plot_filename = "seed_" + num2str(seed(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_seed_" + num2str(seed(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_seed_" + num2str(seed(iterator)) + ".mat";
    ...
    disp("==============================================");
    disp("Currently training with seed: " + num2str(seed(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, seed, settings)
end
