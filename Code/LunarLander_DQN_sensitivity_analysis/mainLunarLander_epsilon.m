...
agent = NaN;
epsilon = [0.999, 0.9];
episodeRewardCell = cell(1,length(epsilon));
averageRewardCell = cell(1,length(epsilon));

for iterator = 1:length(epsilon)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "epsilon.jpg";
    settings.plot_filename = "epsilon_" + num2str(epsilon(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_epsilon_" + num2str(epsilon(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_epsilon_" + num2str(epsilon(iterator)) + ".mat";
    ...
    agent.AgentOptions.DiscountFactor = epsilon(iterator); % !!
    agent.AgentOptions.CriticOptimizerOptions.LearnRate = 1e-3;
    agent.AgentOptions.CriticOptimizerOptions.GradientThreshold = 1;
        
    disp("==============================================");
    disp("Currently training with epsilon: " + num2str(epsilon(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, epsilon, settings)
end
