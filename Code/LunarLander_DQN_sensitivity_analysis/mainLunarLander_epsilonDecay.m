...
agent = NaN;
epsilonDecay = [0.5, 0.005];
episodeRewardCell = cell(1,length(epsilonDecay));
averageRewardCell = cell(1,length(epsilonDecay));

for iterator = 1:length(epsilonDecay)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "epsilonDecay.jpg";
    settings.plot_filename = "epsilonDecay_" + num2str(epsilonDecay(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_epsilonDecay_" + num2str(epsilonDecay(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_epsilonDecay_" + num2str(epsilonDecay(iterator)) + ".mat";
    ...
    agent.AgentOptions.EpsilonGreedyExploration.EpsilonDecay = epsilonDecay(iterator); % !!
    ...
    disp("==============================================");
    disp("Currently training with epsilonDecay: " + num2str(epsilonDecay(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, epsilonDecay, settings)
end
