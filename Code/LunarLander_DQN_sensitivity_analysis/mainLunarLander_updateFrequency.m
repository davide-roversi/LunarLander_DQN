...
agent = NaN;
upFreq = [1, 20];
episodeRewardCell = cell(1,length(upFreq));
averageRewardCell = cell(1,length(upFreq));

for iterator = 1:length(upFreq)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "upFreq.jpg";
    settings.plot_filename = "upFreq_" + num2str(upFreq(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_upFreq_" + num2str(upFreq(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_upFreq_" + num2str(upFreq(iterator)) + ".mat";
    ...
    agent.AgentOptions.TargetUpdateFrequency = upFreq(iterator); % !!
    ...
    disp("==============================================");
    disp("Currently training with upFreq: " + num2str(upFreq(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, upFreq, settings)
end
