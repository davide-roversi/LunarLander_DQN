...
agent = NaN;
NNsize = [64, 512];
episodeRewardCell = cell(1,length(NNsize));
averageRewardCell = cell(1,length(NNsize));

for iterator = 1:length(NNsize)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "NNsize.jpg";
    settings.plot_filename = "NNsize_" + num2str(NNsize(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_NNsize_" + num2str(NNsize(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_NNsize_" + num2str(NNsize(iterator)) + ".mat";
    ...
    net = [
        ...
        fullyConnectedLayer(NNsize(iterator), ...
    ...
    disp("==============================================");
    disp("Currently training with NNsize: " + num2str(NNsize(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, NNsize, settings)
end
