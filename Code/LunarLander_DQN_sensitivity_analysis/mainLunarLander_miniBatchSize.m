...
agent = NaN;
MBS = [64, 256];
episodeRewardCell = cell(1,length(MBS));
averageRewardCell = cell(1,length(MBS));

for iterator = 1:length(MBS)
    
    clear agent
    close all
    
    % Set filenames to save result in
    settings.plot_reward_filename = "MBS.jpg";
    settings.plot_filename = "MBS_" + num2str(MBS(iterator)) + ".jpg";
    settings.data_filename = "/SimOut_Data/trainingStats_MBS_" + num2str(MBS(iterator)) + ".mat";
    settings.agent_filename = "/SimOut_Agents/agent_MBS_" + num2str(MBS(iterator)) + ".mat";
    ...
    agent.AgentOptions.MiniBatchSize = MBS(iterator); % !!
    ...
    disp("==============================================");
    disp("Currently training with MBS: " + num2str(MBS(iterator)));
    disp("==============================================");
...
if settings.plotBadTrajectory == false
    f_rewards_plot(episodeRewardCell, averageRewardCell, MBS, settings)
end
